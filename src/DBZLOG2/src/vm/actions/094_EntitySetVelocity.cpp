// BytecodeVM_EntitySetVelocity -- action opcode 94, 0x0800A526 (0x4C bytes). MEDIUM.
// Pops (character, a, b) and enqueues the vtable-0x08024BF0 command with payload (a, b): sets a target velocity. EntityStopMovement (93) queues the same command with (0, 0).
...
void BytecodeVM_EntitySetVelocity(VmContext* vm, const u8**) {
    const s32 b = Pop(vm);
    const s32 a = Pop(vm);
