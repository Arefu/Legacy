...
## New transformation abilities (added 2026-09-20)
Engine tools > Abilities > "Add transformation ability..." (`DrGero/TransformationAbility.cs`) adds a NEW ability entry that turns the active
character straight into a chosen display row (e.g. Goku base -> a new SSJ God row) for an EP cost, without touching the stock Transformation.

- Stock Transformation (`isReady` 0x080134F2, `onUse` 0x080134C0/0x0801353C, both identical for abilities 11 and 12) only plays animation 0x25
  (0x24 when the row is already flagged "transformed"); the destination is whatever the row's own links say, so it can't reach a second form.
- The switch itself is `BytecodeVM_SetCharacterTransformation` (0x0800A8EC, script opcode 73): pops (slot, spriteId), `CharacterData_FindBySprite`
  (first row with that sprite id), writes the slot's displayDataIndex, refreshes the entity sprite. (HIGH, read from the ROM.)
- Generated code: `isReady(entry)` = currentEp >= cost && displayDataIndex != targetRow (28 bytes); `onUse(entry, entity)` subtracts the EP, builds
  a fake VM stack {2, activeSlot, spriteId} on the Thumb stack and calls that handler with `mov lr` + `bx` (ARM7 has no BLX register; free space is
  also beyond BL range) (60 bytes). Both are written to free space; the ability table is relocated as usual.
- Verified by running the generated bytes from the real (edited) ROM in an emulator against a stub of the handler: EP 80 -> 50, VM words
  (2, 4, sprite), stack pointer and r4 preserved, isReady true/false for the expected cases. NOT verified: playing it in a real emulator.
- "Make revertible" flags the target row as a transformed form (flag bit0) with `detransformedIndex` = a chosen row, so the stock Transformation
  ability reverts it. The target row needs its OWN sprite id (the game finds a form by sprite id, first match); the ability stores that id, so if
  you change the row's sprite later, add the ability again. Only the Default party tab assigns it (new game); it shares the icons of an existing
  ability until you give it its own.
- A new row's frames come from the Sprite editor (export a sprite's PNGs, recolour, import).

### New forms and new sprite ids (added 2026-09-20)
- "Add transformation ability..." can also **create the form**: with "Create it as a NEW form" it copies the chosen row (e.g. Goku's SSJ row 20) into a
  new display row with a NEW sprite id, flags it as a transformed form that reverts to a chosen row, and adds the ability for it. The new sprite is
  a copy of the source sprite's record, so its frames start identical; export its PNGs in the Sprite editor, recolour, and import them back.
- New sprite ids need room in `g_CharacterSpriteIndex` (file 0x3B4E74, ids 3..156 in use, followed by unrelated data). Its only reference is ONE code
  literal: `Character_GetSpriteId` (0x08009324) loads 0x083B4E90 (= table + 7 words) from file offset 0x959C and indexes it with no bounds check
  (HIGH: no other word in the ROM points near the table). `FrameImport.CloneSprite` relocates the table to free space (256 ids) and patches that
  literal; `FrameImport.IndexAddress(rom)` reads it back, so every DBZKit tool follows a relocated table. Verified by running the game's own
  `Character_GetSpriteId` on the edited ROM in an emulator: ids 7, 61, 156 and the new 157 all resolve to the right records.
- Headless check of the whole chain on the real ROM: clone sprite 61 -> id 157 (272 frames, 0 differing pixels), new row 22 (sprite 157, flag 1,
  reverts to 19), ability 14; importing an edited dump into sprite 157 leaves sprite 61 untouched.

### Sprite records are not a fixed size (found 2026-09-20)
A record is the 0x4C-byte header followed by as many 16-byte groups (Down/Up/Left/Right frame-array pointers) as the sprite has: 21 groups for 140 of the 150
sprites, 22 for 7, 23 for 3 (sprite 61, Goku SSJ, has 22). The record ends at the first non-zero word that is not a frame pointer (its descriptor arrays follow
it). Earlier DBZKit builds copied a fixed 0x1A0 bytes when relocating or cloning a record, which silently dropped the groups beyond 0x1A0 for those 10 sprites,
and the dump / editor stopped at group 19, hiding group 20 (0x18C, "extra frames": 662 more frames across all sprites; possibly the idle / blink frames, not
confirmed). `FrameImport.RecordExtent / RecordLimit / GroupsIn` now size everything from the record itself, and the Sprite editor's group list is exactly the
groups the dump walks for the picked sprite (it opens on the first sprite and the first group).
