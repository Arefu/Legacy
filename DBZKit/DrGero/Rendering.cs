...
namespace DrGero.Rendering
{
    public static class EntityReader
    {
        ...
        public static List<Entity> ReadMapTriggers(ROM rom, MapEntry entry)
        {
            ...
            for (int i = 0; i < entry.TriggerCount; i++)
            {
                ...
                if (dataPtr != 0)
                {
                    ...
                    rom.Skip(8); // condition + function pointers, common to all 4 trigger variants
                    int x = (short)rom.ReadShort();
                    int y = (short)rom.ReadShort();
                    rom.PopPosition();

                    result.Add(new Entity(EntityKind.Trigger, x, y, vTable, entryAddress));
                }
            }
        ...
        public static List<Entity> ReadNpcArray(ROM rom, MapEntry entry)
        {
            var result = new List<Entity>();
            ...
            for (int i = 0; i < entry.NpcCount; i++)
            {
                ...
                int x = rom.ReadShort();
                int y = rom.ReadShort();
                int index = rom.ReadByte();
                int type = rom.ReadByte();
                int unk06 = rom.ReadByte();
                int flags = rom.ReadByte();

                result.Add(new Entity(EntityKind.Npc, x, y, type, entryAddress));
            }
