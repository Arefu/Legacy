...
namespace DBZKit
{
    internal static class GBA
    {
        ...
        /// <summary>
        /// Renders 8bpp GBA tiled sprite data. Tiles are 8x8, stored row by row within each tile.
        /// </summary>
        public static Bitmap Render8bppTiled(byte[] data, int widthInTiles, int heightInTiles, Color[]? palette = null)
        {
            int width = widthInTiles * 8;
            int height = heightInTiles * 8;
            var bmp = new Bitmap(width, height, PixelFormat.Format32bppArgb);
            ...
            for (int tileY = 0; tileY < heightInTiles; tileY++)
            {
                for (int tileX = 0; tileX < widthInTiles; tileX++)
                {
                    int tileOffset = tileIndex * 64; // 64 bytes per 8x8 8bpp tile
                    for (int row = 0; row < 8; row++)
                    {
                        for (int col = 0; col < 8; col++)
                        {
                            byte index = data[tileOffset + row * 8 + col];
                            int px = tileX * 8 + col;
                            int py = tileY * 8 + row;
                            Color color = palette != null ? palette[index] : Color.FromArgb(index, index, index);
